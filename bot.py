...
# Готовые ответы по актуальному CV. Они сохраняют прежнее меню FAQ даже тогда,
# когда внешний LLM/RAG временно недоступен (например, из-за региона VPS).
FALLBACK_FAQ_REPLIES: Dict[str, str] = {
    "who_now": (
        "С мая 2026 года Тимур — старший технический менеджер направления Алиса AI и умных "
        "устройств в Яндексе. Выстраивает R&amp;D-процесс и ведёт проекты новых устройств от запроса "
        "продуктовой команды до результата. Целевой профиль: CTO, CIO, CAIO и технический директор."
    ),
    "industry": (
        "Основные домены: AI/ML и LLM, embedded-системы, IoT, телеком и магистральная связь, "
        "промышленная и потребительская электроника. Есть опыт в энергетике, нефтегазе, "
        "госсекторе и системах безопасности."
    ),
    "company_scale": (
        "Работал и в крупных экосистемах, и в технологических компаниях: Яндекс, Сбербанк, Т8 и "
        "Натекс. Лаборатория IoT Сбербанка — около 30 человек; в резюме также указан масштаб до "
        "100 человек в прямом подчинении и до 2000 участников работы над продуктом."
    ),
    "scope": (
        "Зона ответственности охватывает техническую и продуктовую стратегию, R&amp;D-процессы, "
        "бюджетирование и ТЭО, разработку электроники и AI-решений, сертификацию и полный цикл "
        "от технического задания до массового производства. R&amp;D-бюджеты — до 5 млн долларов в год."
    ),
    "team": (
        "Опыт управления — до 100 человек в прямом подчинении и до 2000 участников работы над "
        "продуктом. В отдельных ролях: лаборатория Сбербанка — около 30 человек, Т8 — 30+, "
        "Натекс — до 30, производственная команда Неоторга — до 100 человек."
    ),
    "skills": (
        "Ключевые компетенции: техническая и продуктовая стратегия, построение R&amp;D-процессов, "
        "AI-трансформация, бюджетирование и ТЭО, масштабирование команд, вывод hardware-продуктов "
        "в серию, embedded, IoT, телеком, компьютерное зрение и речевые технологии."
    ),
    "achievements": (
        "— В 2 раза увеличил оборот Т8.\n"
        "— Сократил расходы на разработку на 30% в Т8 и затраты R&amp;D-отдела на 30% в Натекс.\n"
        "— Вдвое сократил time-to-market и повысил точность планирования с 60% до 85%.\n"
        "— Повысил продуктивность подразделения Сбербанка на 20% за счёт AI.\n"
        "— Продукт под его руководством получил премию CES 2022."
    ),
    "location": (
        "Тимур находится в Москве, имеет гражданство России и готов к переезду и командировкам. "
        "Русский — родной, английский — C1."
    ),
    "legal": (
        "В резюме указано гражданство России и готовность к переезду и командировкам. Данные о "
        "зарубежном разрешении на работу, non-compete и сроке выхода не указаны — их лучше уточнить лично."
    ),
    "pivots": (
        "В Сбербанке развивал применение AI и LLM в реальных продуктах и новых модальностях. В Т8 "
        "оптимизировал продуктовую линейку и завершил либо закрыл замороженные проекты. В Натекс "
        "вывел новые продуктовые направления и внедрил матричную структуру управления."
    ),
    "from_scratch": (
        "Создавал и перестраивал функции с нуля: выстраивал R&amp;D-процессы, формировал SW- и "
        "HW-команды для проекта UMPC, организовал сборочное производство на новой базе и создавал "
        "функциональные группы с прозрачной системой отчётности и планирования."
    ),
    "crisis": (
        "В Т8 завершил либо ликвидировал ранее замороженные проекты, высвободив ресурсы, и сократил "
        "расходы на разработку на 30%. Автоматизация отчётности и корректировки планов снизила риск "
        "срыва сроков и сделала этапы проектов прозрачнее."
    ),
    "global": (
        "Есть опыт международных переговоров и партнёрств в Китае, Гонконге, Финляндии и Германии, "
        "а также взаимодействия с Intel, VIA, Qualcomm, LG-Philips-LCD, Touch International и AMI. "
        "Работал с контрактными производителями в России и Азии; английский — C1."
    ),
    "stakeholders": (
        "Работал с руководителями и продуктовыми командами, инвесторами, государственными "
        "заказчиками и ведомствами, крупными корпоративными клиентами, научными школами и "
        "технологическими партнёрами. В числе клиентов — Сбербанк-ЦОД, Транснефть, Ростелеком, "
        "Мегафон, Билайн и МТС."
    ),
    "reputation": (
        "Продукт, созданный под руководством Тимура, стал лауреатом CES 2022. В резюме указаны "
        "патенты, публикации, выступления на AI Journey и в Корпоративном университете, а также "
        "мировые рекорды продукции Т8 по дальности и скорости передачи данных."
    ),
    "pnl": (
        "Управлял R&amp;D-бюджетом до 5 млн долларов в год, включая государственное финансирование. "
        "Отвечал за выпуск до 3000 изделий в год стоимостью свыше 3 млрд рублей и обеспечил "
        "двукратный рост оборота Т8. Полная ответственность за финансовый результат или EBITDA "
        "в резюме не заявлена."
    ),
    "system": (
        "Выстраивал R&amp;D-процессы, матричную структуру, функциональные группы, системы отчётности, "
        "планирования и мотивации. В Натекс точность планирования выросла с 60% до 85%, а скорость "
        "выполнения задач после организационных изменений — на 20%."
    ),
    "leadership": (
        "Формировал и масштабировал инженерные, R&amp;D-, SW-, HW- и производственные команды. В Т8 "
        "увеличил команду в 1,5 раза за счёт специалистов с уникальной экспертизой; также создавал "
        "системы материальной и нематериальной мотивации."
    ),
    "change": (
        "Примеры измеримых изменений: рост продуктивности подразделения Сбербанка на 20% благодаря "
        "AI-трансформации, сокращение расходов на разработку на 30%, двукратное сокращение "
        "time-to-market и рост точности планирования с 60% до 85%."
    ),
}
...
def load_cache():
    ...
    # Динамический кэш мог не создаться из-за недоступности OpenAI. В таком
    # случае показываем тот же HR-каталог с проверенными ответами из актуального CV.
    if not ACTIVE_FAQ_TOPICS:
        for item in hr_faq_catalog():
            reply = FALLBACK_FAQ_REPLIES.get(item["key"], "").strip()
            if reply:
                ACTIVE_FAQ_TOPICS.append((item["key"], item["label"], item["full"]))
                FAQ_CACHE[item["key"]] = reply
